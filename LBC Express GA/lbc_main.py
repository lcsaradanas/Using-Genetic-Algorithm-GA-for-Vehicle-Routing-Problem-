...
distance_matrix = {
    ('Pasay', 'QC'): 8, ('QC', 'Pasay'): 8,
    ('QC', 'Makati'): 9, ('Makati', 'QC'): 9,
    ('Pasay', 'Makati'): 10, ('Makati', 'Pasay'): 10,
    ('Mandaluyong', 'Manila'): 7, ('Manila', 'Mandaluyong'): 7,
    ('Taguig', 'Makati'): 5, ('Makati', 'Taguig'): 5,
    ('Makati', 'Pasig'): 7, ('Pasig', 'Makati'): 7,
    ('Pasig', 'Manila'): 9, ('Manila', 'Pasig'): 9,
    ('San Juan', 'QC'): 6, ('QC', 'San Juan'): 6,
    ('QC', 'Pasig'): 5, ('Pasig', 'QC'): 5,
    ('Pasay', 'Taguig'): 8, ('Taguig', 'Pasay'): 8
}
...
def per_vehicle_scores_lbc(best_routes, vehicles, distance_matrix, alpha=1, beta=1, gamma=10, delta=10, export_csv=False):
    results = []
    print("\nPer-vehicle fitness breakdown for LBC:")
    for v in vehicles:
        route = best_routes[v['id']]
        # Calculate the total distance by summing up all segments
        distance = 0
        for i in range(len(route) - 1):
            distance += distance_matrix.get((route[i], route[i+1]), 0)
        time = distance / average_speed * 60
        over_cap = v['over_capacity']
        late = v['late_delivery']
        indiv_score = alpha * distance + beta * time + gamma * over_cap + delta * late
        results.append({
            'ID': v['id'],
            'Route': ' -> '.join(route),
            'Distance': distance,
        ...
        print(f"{v['id']} | Route: {route} | Distance: {distance}km | TravelTime: {time}min | OverCapacity: {over_cap} | Late: {late} | Fitness: {indiv_score}")
    if export_csv:
        pd.DataFrame(results).to_csv('per_vehicle_fitness_lbc.csv', index=False)
        print("\nCSV file 'per_vehicle_fitness_lbc.csv' exported!")
    return results

if __name__ == "__main__":
    best_routes, best_fitness = genetic_algorithm(vehicles, distance_matrix)
    print("\nBest Route Assignment:", best_routes)
    print("Fitness Score:", best_fitness)
    per_vehicle_scores_lbc(best_routes, vehicles, distance_matrix, alpha=1, beta=1, gamma=10, delta=10, export_csv=True)
